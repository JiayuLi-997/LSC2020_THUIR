...
class MyScorer(scoring.WeightLengthScorer):
    ...
    def _score(self, weight, length,poses=[]):
        # if len(poses) == 0:
        #     print(self.fieldname,self.text)
        ...
        # print('calculate tf: ',len(poses)/length)
        # print(self.idf)
        s = bm25fun(self.idf, pos_w, length, self.avgfl, self.B, self.K1)
        # s = bm25fun(self.idf, weight, length, self.avgfl, self.B, self.K1)
        # s = s + 1/(poses[0]+1) if len(poses) else s
        # s = s + sum([1/(int(x/10)+1) for x in poses]) if len(poses) else s
        # print(s)
        return s
...
#     def scorer(self, searcher, fieldname, text, qf=1):
#         return MyScorer(searcher, fieldname, text, qf=qf)
...
#         Id = matcher.id()
#         poses = matcher.value_as("positions")
#         weight_score = sum([1/(int(x/10)+2) for x in poses])
#         weight_score = 1/(poses[0]+1)
#         return weight_score
...
class SearchClass:
    ...
    def __init__(self, ix_dir="./blueprints/index_updatemar9_dir",weighting_type='BM25F',floor=30,k=1,feedback_type="score"):
        ...
        b_time = tm.time()
        # print('searcher initialize: ',b_time-a_time,'s')
